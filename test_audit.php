...
$tx = new Transaction;
$tx->method = 'wave';
check('wave = Wave', $tx->method_label, 'Wave');
$tx->method = 'orange_money';
check('orange_money = Orange Money', $tx->method_label, 'Orange Money');
$tx->method = 'free_money';
check('free_money = Free Money', $tx->method_label, 'Free Money');
$tx->method = 'card';
check('card = Carte bancaire', $tx->method_label, 'Carte bancaire');
$tx->method = 'cash';
check('cash = Espèces', $tx->method_label, 'Espèces');
$tx->method = 'direct_transfer';
check('direct_transfer = Transfert P2P', $tx->method_label, 'Transfert P2P');
$tx->method = 'unknown';
check('unknown = ucfirst fallback', $tx->method_label, 'Unknown');

$tx->status = 'success';
check('success = Payé', $tx->status_label, 'Payé');
$tx->status = 'pending';
check('pending = En attente', $tx->status_label, 'En attente');
$tx->status = 'failed';
check('failed = Échoué', $tx->status_label, 'Échoué');
$tx->status = 'reversed';
check('reversed = Remboursé', $tx->status_label, 'Remboursé');
$tx->status = 'cancelled';
check('cancelled = Annulé', $tx->status_label, 'Annulé');
...
if ($user) {
    ...
    check('referrals() retourne collection', $user->referrals() instanceof HasMany, true);
} else {
...
foreach ($services as $svc) {
    ...
    try {
        app($svc);
        check("$name instanciable", true);
    } catch (Throwable $e) {
        check("$name instanciable", false);
        echo '       → '.$e->getMessage()."\n";
    }
}
...
foreach ([
    ProcessCycle::class,
    RecalculateCreditScore::class,
    SendReminders::class,
    SendChatNotifications::class,
] as $job) {
...
// ─── 9. WEBHOOK OUTBOUND ──────────────────────────────────────────────────
echo "\n── WEBHOOK OUTBOUND ──\n";
$wb = app(WebhookOutboundService::class);
try {
    $wb->dispatch('test.event', ['foo' => 'bar']);
    check('dispatch sans URL ne plante pas', true);
} catch (Throwable $e) {
    check('dispatch sans URL ne plante pas', false);
    echo '       → '.$e->getMessage()."\n";
}
...
if ($user = User::first()) {
    try {
        $cs = app(CreditScoringService::class);
        $creditScore = $cs->calculate($user);
        check('calculate retourne un CreditScore', $creditScore instanceof CreditScore, true);
...
if ($user = User::first()) {
    try {
        $gs = app(GamificationService::class);
        ...
        check('getLeaderboardForUser retourne collection', $lb instanceof Collection, true);
    } catch (Throwable $e) {
...
check('EVENT_REFERRAL défini', NotificationService::EVENT_REFERRAL === 'referral_joined', true);
